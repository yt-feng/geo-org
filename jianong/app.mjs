...
const state = { user: null, config: null, view: 'chat', mode: 'sales', conversationId: null, conversations: [], messages: [], authMode: 'login', captchaId: '', knowledgeTab: 'documents', knowledge: null, customers: [], customer: null, customerId: null, editCustomerId: null, pendingChat: false, imagePolls: new Map(), adminAction: null, paddleReady: false, social: null, customerFilter: 'active', lastChatRequest: null, lastImageRequest: null, checkoutRequests: new Map(), imageAdmin: null };
...
async function api(path, { method = 'GET', body, timeout = 95000, silent = false } = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 && !silent && !['/login', '/register', '/password'].includes(path)) updateUser(null);
      throw error;
    }
    return data;
  } catch (error) {
...
function updateUser(user) {
  ...
  if (previousId && previousId !== user?.id) {
    state.conversationId = null; state.messages = []; state.conversations = []; state.customers = []; state.customer = null; state.customerId = null; state.lastChatRequest = null; state.lastImageRequest = null; state.checkoutRequests.clear();
    empty($('image-results')); empty($('customer-detail-body')); empty($('activity-list'));
    state.imagePolls.forEach((token) => { token.cancelled = true; }); state.imagePolls.clear();
    empty($('messages')); empty($('image-history')); empty($('customer-list')); empty($('admin-users')); empty($('ledger-body'));
    renderHistory(); updateChatIntro(); renderCustomerContext();
  }
  updateWalletSummary();
...
async function sendChat(event) {
  ...
  } finally { state.pendingChat = false; $('send-button').disabled = false; $('chat-input').focus(); }
}
async function loadHistory() {
  if (!state.user) { state.conversations = []; renderHistory(); return; }
  try { const result = await api('/conversations'); state.conversations = result.conversations || []; renderHistory(); } catch (error) { setText('history-list', error.message); }
}
...
async function refreshCaptcha() {
  ...
  catch (error) { setText('captcha-loading', '点击重试'); status('auth-status', error.message, true); }
  finally { $('refresh-captcha').disabled = false; }
}
async function submitAuth(event) {
  ...
  try {
    ...
    const result = await api(`/${state.authMode}`, { method: 'POST', body, silent: true }); updateUser(result.user); closeDialog('auth-dialog'); $('auth-password').value = ''; $('auth-captcha').value = ''; toast(state.authMode === 'register' ? '欢迎加入，开始你的第一段销售对话吧。' : '登录成功，欢迎回来。'); await loadHistory(); await switchView(state.view);
  } catch (error) { const message = error.message; await refreshCaptcha(); status('auth-status', message, true); }
  finally { busy(form, false); }
}
async function refreshMe() { try { const result = await api('/me', { silent: true }); updateUser(result.user); return result.user; } catch (error) { if (error.status === 401) updateUser(null); return null; } }
...
async function logout() { try { await api('/logout', { method: 'POST', body: {} }); updateUser(null); closeDialog('account-dialog'); newChat(); toast('已退出登录。'); } catch (error) { toast(error.message); } }
...
async function loadKnowledge() {
  ...
  catch (error) { status('knowledge-status', error.message, true); empty(container); }
}
...
async function pollImage(id) {
  ...
  const token = { cancelled: false }; state.imagePolls.set(id, token);
  try {
    for (let attempt = 0; attempt < 20; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 15000)); if (token.cancelled || !state.user) return;
  ...
  } catch (error) { status('image-status', `${error.message} 已提交的作品可在下方继续查看。`, true); }
  finally { state.imagePolls.delete(id); }
}
...
function renderPackages() {
  ...
  setText('billing-note', state.config?.paddle?.enabled ? '通过 Paddle 结账。价格、货币与税费以结账页为准，付款确认后积分自动到账。' : 'Paddle 支付正在开通。开通后即可在线购买积分；已有体验积分可正常使用。');
}
...
let paddleLoading;
async function preparePaddle() {
  if (state.paddleReady) return true;
  const config = state.config?.paddle;
  if (!config?.enabled || !config.client_token) return false;
  if (!paddleLoading) paddleLoading = new Promise((resolve, reject) => {
    const finish = () => { try { if (config.environment === 'sandbox') window.Paddle.Environment.set('sandbox'); window.Paddle.Initialize({ token: config.client_token, eventCallback: (event) => { if (event.name === 'checkout.completed') { state.checkoutRequests.clear(); toast('付款已提交，正在等待积分到账。'); refreshAfterPayment(); } } }); state.paddleReady = true; resolve(true); } catch (error) { reject(error); } };
    if (window.Paddle) { finish(); return; }
    const script = document.createElement('script'); script.src = 'https://cdn.paddle.com/paddle/v2/paddle.js'; script.async = true; script.addEventListener('load', finish, { once: true }); script.addEventListener('error', () => reject(new Error('支付组件暂时加载失败，请稍后重试。')), { once: true }); document.head.append(script);
  }).catch((error) => { paddleLoading = null; throw error; });
  return paddleLoading;
}
async function startCheckout(packageId, button) {
  if (!requireUser()) return;
  if (!state.config?.paddle?.enabled) { toast('Paddle 支付正在开通，开通后即可在线充值。'); return; }
  button.disabled = true;
  try { const result = await api('/checkout', { method: 'POST', body: { package_id: packageId, request_id: state.checkoutRequests.get(packageId) || (state.checkoutRequests.set(packageId, newId()), state.checkoutRequests.get(packageId)) } }); const prepared = await preparePaddle(); if (prepared && result.transaction_id) { window.Paddle.Checkout.open({ transactionId: result.transaction_id, settings: { displayMode: 'overlay', theme: 'light', locale: 'zh-Hans' } }); } else { const url = safeUrl(result.checkout_url, false); if (!url) throw new Error('结账链接尚未准备好，请稍后重试。'); window.location.assign(url); } }
  catch (error) { toast(error.message); }
  finally { button.disabled = false; }
}
async function refreshAfterPayment() { const initial = Number(state.user?.credits || 0); for (let i = 0; i < 15; i += 1) { await new Promise((resolve) => setTimeout(resolve, 2000)); if (!state.user) return; await refreshMe(); if (Number(state.user?.credits || 0) > initial) { toast('充值积分已到账。'); if (state.view === 'wallet') await loadWallet(); return; } } toast('付款正在确认中，积分以账户流水为准，可稍后刷新余额。'); }

async function loadAdmin() {
  if (!isAdmin()) return;
  await loadAdminStats(); status('admin-status', '正在读取团队成员…');
  ...
  } catch (error) { status('admin-status', error.message, true); }
}
async function adminAction(user, action) {
  if (['disable', 'enable'].includes(action)) { try { await api(`/admin/users/${encodeURIComponent(user.id)}`, { method: 'POST', body: { action } }); toast(action === 'disable' ? '账号已停用。' : '账号已启用。'); await loadAdmin(); } catch (error) { toast(error.message); } return; }
...
async function submitAdminAction(event) {
  ...
  catch (error) { status('admin-action-status', error.message, true); }
  finally { busy(event.currentTarget, false); }
}
...
async function loadCustomers() {
  ...
  } catch (error) { status('customer-status', error.message, true); }
}
...
async function saveCustomer(event) {
  ...
  try { const data = Object.fromEntries(new FormData(event.currentTarget)); data.raw_text = $('customer-raw').value.trim(); data.next_follow_up = inputDate(data.next_follow_up); const id = state.editCustomerId; await api(id ? `/customers/${encodeURIComponent(id)}` : '/customers', { method: id ? 'PATCH' : 'POST', body: data }); closeDialog('customer-dialog'); toast('客户资料已保存。'); await loadCustomers(); if (id) await openCustomer(id); }
  catch (error) { status('customer-form-status', error.message, true); }
  finally { busy(event.currentTarget, false); }
}
...
async function saveActivity(event) {
  ...
  try { await api(`/customers/${encodeURIComponent(state.customer.id)}/activities`, { method: 'POST', body: { content: $('activity-content').value.trim(), next_follow_up: inputDate($('activity-next').value), stage: $('activity-stage').value || state.customer.stage } }); toast('跟进记录已保存。'); await openCustomer(state.customer.id); await loadCustomers(); }
  catch (error) { status('activity-status', error.message, true); }
  finally { busy(event.currentTarget, false); }
}
...
async function submitImageAdmin(event) {
  ...
  catch (error) { status('image-admin-status', error.message, true); }
  finally { busy(event.currentTarget, false); }
}
...
async function loadListening() {
  ...
  try {
    ...
    renderSocialAssets();
  } catch (error) { status('listening-status', error.message, true); }
}
