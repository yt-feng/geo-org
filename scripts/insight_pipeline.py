...
选项互斥且无遗漏，并同步核对摘要、表格和执行路径。对每一行示例，从输入开始执行
全部有序分支（含新增的不确定区间、差值门槛和相等时规则），独立算出最终选项，
再核对表中的推荐；不能只核对公式数值而漏掉后续分支推翻了推荐。
...
新增差值门槛、不确定区间或平局分支时，逐行把全部示例输入代入完整的有序规则，
重新给出最终选项；若新增规则让原反转示例不再成立，必须同步改示例或删除无依据
的新规则。期望值之间与实测值之间分别比较，不得把一个方案的实测值与另一个
方案的期望值混合排序；区间内外的结论与统计证据边界必须在出现处说清楚。
