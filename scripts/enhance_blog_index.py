...
<body>
  ...
  <footer class="footer site-footer"><div class="wrap">© 2026 Eco GEO · Brand-first GEO · <a href="../index.html">首页</a> · <a href="../blog/">Blog</a> · <a href="mailto:yt.feng@foxmail.com">yt.feng@foxmail.com</a></div></footer>
  <script>
    const PAGE_SIZE = 24;
    const IMAGE_POOL = [
    ...
    function cardHtml(p, imageIndex){ const url = `articles/${encodeURIComponent(p.slug)}/`; const img = imageFor(p, imageIndex); return `<a class="card" href="${url}"><div class="thumb"><img src="${img}" alt="${escapeAttr(p.title)}" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display='none'"></div><div class="card-body"><div class="meta"><span>${escapeHtml(p.category)}</span><span>${escapeHtml(p.author)}</span></div><h2>${escapeHtml(p.title)}</h2><p>${escapeHtml(p.excerpt)}</p></div></a>`; }
