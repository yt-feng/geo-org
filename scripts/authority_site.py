...
TRUST_PAGES = {
    ...
    "privacy": {
        "zh": {"title": "隐私政策", "lead": "我们仅使用你主动提交的信息来处理和回复咨询，不出售这些信息。", "sections": [("你提交的信息", ["咨询表单接收你主动填写的姓名、邮箱、公司或品牌、网站和咨询内容，其中公司或品牌和网站为选填。", "品牌评测 Demo 在浏览器端运行，不上传评测输入。", "预算预览在浏览器内计算。点击“生成 AI 定制建议”后，你填写的预算、服务市场、服务语种、业务范围、选配项目、聆听需求、目标、品牌基础和备注经 Cloudflare 服务端发送至 DeepSeek，用于生成本次建议。本站不将这些输入和建议存入咨询数据库，也不会自动发送咨询邮件；服务提供商可能依其政策处理请求与运行日志。选择携带方案咨询时，摘要仅在当前标签页的会话中临时保留，30 分钟后失效，并在咨询页面读取时移除；确认提交表单后才会发送咨询。"]), ("处理与保存", ["表单通过 Cloudflare 处理并发送邮件通知。我们通过邮件处理和回复咨询，不将咨询内容存入网站咨询数据库。", "你的联系方式和咨询内容仅用于回应此次咨询，不出售给第三方。邮件服务会为收件和回复保存邮件。"]), ("第三方资源", ["站点可能加载 Unsplash 图片和 Vercel 托管资源；Cloudflare、邮件服务及其他第三方可能依据自身政策处理提供服务所需的数据与访问日志。"]), ("联系", [f"隐私相关问题请联系 {EMAIL}。"])]},
        "en": {"title": "Privacy Policy", "lead": "We use the information you choose to submit only to handle and respond to your inquiry. We do not sell it.", "sections": [("Information You Submit", ["The inquiry form receives the name, email address, company or brand, website, and message you provide. Company or brand and website are optional.", "The brand audit demo runs in your browser and does not upload audit inputs.", "Budget previews are calculated in your browser. When you request an AI recommendation, your budget, service market, service languages, business scope, selected modules, listening requirements, goal, brand stage and notes are sent through our Cloudflare service to DeepSeek to generate the recommendation. We do not store these inputs or recommendations in an inquiry database or automatically send inquiry emails. Service providers may process requests and operational logs under their policies. If you choose to take your plan to consultation, a summary is kept temporarily in this tab session, expires after 30 minutes, and is removed when the contact page reads it. An inquiry is sent only when you submit the form."]), ("Processing and Storage", ["Cloudflare processes form submissions and sends email notifications. We handle and reply to inquiries by email and do not store inquiry content in a website inquiry database.", "Your contact details and message are used only to respond to your inquiry and are not sold to third parties. Email services retain messages for receiving and replying to inquiries."]), ("Third Parties", ["The site may load Unsplash images and Vercel-hosted resources. Cloudflare, email services, and other providers may process service data and access logs under their own policies."]), ("Contact", [f"Privacy questions: {EMAIL}."])]},
        "ar": {"title": "سياسة الخصوصية", "lead": "نستخدم المعلومات التي تختار إرسالها فقط لمعالجة استفسارك والرد عليه، ولا نبيعها.", "sections": [("المعلومات التي ترسلها", ["يستقبل نموذج الاستفسار الاسم والبريد الإلكتروني واسم الشركة أو العلامة والموقع الإلكتروني والرسالة التي تقدمها. اسم الشركة أو العلامة والموقع الإلكتروني اختياريان.", "تعمل تجربة تقييم العلامة داخل متصفحك ولا ترفع مدخلات التقييم إلى الخادم.", "تُحسب معاينة الميزانية داخل متصفحك. عند طلب توصية بالذكاء الاصطناعي، تُرسل الميزانية والسوق ولغات الخدمة ونطاق العمل والخدمات المختارة ومتطلبات الاستماع والهدف ومرحلة العلامة والملاحظات عبر خدمة Cloudflare إلى DeepSeek لإعداد التوصية. لا نخزن هذه المدخلات أو التوصيات في قاعدة بيانات للاستفسارات، ولا نرسل رسائل استفسار تلقائياً. وقد يعالج مقدمو الخدمات الطلبات وسجلات التشغيل وفق سياساتهم. عند اختيار نقل الخطة إلى نموذج الاستشارة، يُحفظ ملخص مؤقت في جلسة علامة التبويب الحالية لمدة 30 دقيقة، ويُحذف عند قراءته في صفحة التواصل. لا يُرسل الاستفسار إلا بعد إرسال النموذج."]), ("المعالجة والحفظ", ["تعالج Cloudflare بيانات النموذج وترسل إشعارات بالبريد الإلكتروني. نعالج الاستفسارات ونرد عليها عبر البريد، ولا نخزن محتواها في قاعدة بيانات للاستفسارات على الموقع.", "تستخدم بيانات الاتصال ورسالتك فقط للرد على استفسارك ولا تباع لأطراف ثالثة. تحتفظ خدمات البريد بالرسائل لاستلام الاستفسارات والرد عليها."]), ("أطراف ثالثة", ["قد يحمل الموقع صور Unsplash وموارد مستضافة على Vercel. وقد تعالج Cloudflare وخدمات البريد ومقدمو الخدمات الآخرون البيانات اللازمة للخدمة وسجلات الوصول وفق سياساتهم."]), ("التواصل", [f"لأسئلة الخصوصية: {EMAIL}."])]},
    },
    "terms": {
