...
译稿的独立审稿若提出原文事实问题，同一次运行最多自动返回中文修订一次：先验证选题、来源指纹及原文签名，保留全部中英阿稿件于 `localization-history`，将阻断问题交给中文作者及独立审稿逐项核验，再重新生成两种译稿。单纯数字形式/HTML问题不触发中文重写；服务、凭据和程序错误不会进入内容修订。修复后仍不通过则停止并保留审计，三语发布门槛不变。接受阈值须明确等号边界；未读取到统计显著性标记不能改写为“无统计显著性”。
...
翻译保持原文数字与文字数词形式，逐字段比较数值及出现数量；缺失/新增值与数量回传到修订提示。仅将阿语方向标记、正规千位分隔和等价百分比写法作校验归一化，不改正文，不放宽数值变化。译稿单独设置较高输出上限以容纳完整阿语及思考输出；模型正常完成即停止，上限不是目标长度。参见 [DeepSeek 输出与思考参数](https://api-docs.deepseek.com/api/create-chat-completion/)。
