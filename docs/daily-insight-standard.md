...
每轮修订保留所有历史普通修改意见、结构问题和阻断项，沿用原修订ID；审稿不再提及某项不代表该项自动解决。原始提纲未经独立验证，只保留研究脉络，不能在后续修订中恢复已被纠正的数字。定量写作与独立审稿均核对完整工时账、共同前置工作的成本和产出、修复空间耗尽后的剩余工时、模型期望值与实测整数的区别，以及零值、相等、超预算条件下互斥且完整的行动规则。历史阻断项仍由独立审稿逐项确认，所有质量门槛保持不变。
